...
import { QuestionCard, Button, Alert, Badge } from '../../components';
...
export default function ResultReview() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-bg-primary flex flex-col items-center justify-center text-text-primary">
        ...
        <p className="text-sm text-text-secondary">Grading mock answers and preparing solution map...</p>
      </div>
    );
  }
  ...
  if (error || !result) {
    return (
      <div className="min-h-screen bg-bg-primary flex flex-col items-center justify-center text-text-primary p-6">
        ...
        <p className="text-sm text-text-secondary mb-6 text-center max-w-sm">{error || 'Result details missing.'}</p>
        <button
  ...
  return (
    <div className="min-h-screen bg-bg-primary text-text-primary font-sans pb-16 relative overflow-auto">
      {/* Decorative Orbs */}
      ...
      {/* Header */}
      <header className="border-b border-border bg-bg-primary backdrop-blur-md sticky top-0 z-40">
      ...
      <main className="mx-auto max-w-6xl px-6 py-8 space-y-8 relative z-10">
        ...
        <section className="grid gap-6 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-primary/20 bg-bg-secondary/20 p-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
            ...
            <div className="mt-2 text-5xl font-black text-text-primary">
              {result.type === 'mock' ? (
                <>
                  {result.totalScore}
                  <span className="text-sm text-text-secondary font-normal"> / 400</span>
                </>
              ) : (
                `${result.totalScore}%`
              )}
          ...
          {/* Timing Stats Card */}
          <div className="rounded-2xl border border-border bg-bg-secondary p-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <Clock className="h-5 w-5 text-violet-400" />
                <span className="text-sm font-bold text-text-primary">Solve Speed Overview</span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="text-[10px] uppercase font-semibold text-text-muted">Duration Taken</div>
                  <div className="text-xl font-extrabold text-text-primary mt-0.5">{durationFormatted}</div>
                </div>
                <div>
                  <div className="text-[10px] uppercase font-semibold text-text-muted">Average Pace</div>
            ...
            <div className="mt-6 text-[10px] text-text-muted leading-normal">
          ...
          <div className="rounded-2xl border border-border bg-bg-secondary p-6 flex flex-col justify-between">
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-text-primary">Answer Accuracies</span>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-lg bg-emerald-500/5 border border-emerald-500/10 p-2">
                  ...
                  <div className="text-[8px] text-text-muted uppercase mt-0.5">Correct</div>
                </div>
                <div className="rounded-lg bg-rose-500/5 border border-rose-500/10 p-2">
                  ...
                  <div className="text-[8px] text-text-muted uppercase mt-0.5">Wrong</div>
                </div>
                <div className="rounded-lg bg-bg-secondary border border-border p-2">
                  ...
                  <div className="text-[8px] text-text-muted uppercase mt-0.5">Skipped</div>
                </div>
              </div>
            </div>
            <div>
              ...
              <div className="w-full bg-bg-primary rounded-full h-1.5 overflow-hidden border border-border">
        ...
        <section className="rounded-2xl border border-border bg-bg-secondary p-6 space-y-4">
          <div className="flex items-center gap-2 pb-2 border-b border-border">
            <BookOpen className="h-5 w-5 text-primary" />
            <h3 className="text-sm font-bold text-text-primary">Performance Scorecard per Subject</h3>
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {result.subjectScores.map((scoreCard) => (
              <div
                key={scoreCard.subject}
                className="rounded-xl border border-border bg-bg-primary p-4 flex items-center justify-between"
              >
                <div>
                  <div className="text-sm font-bold text-text-primary">{SUBJECT_LABELS[scoreCard.subject] || scoreCard.subject}</div>
        ...
        <section className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between pb-4 border-b border-border">
          <div className="flex items-center gap-2">
            <ListCollapse className="h-4 w-4 text-primary" />
            <h3 className="text-sm font-bold text-text-primary">Correction Map Corrections</h3>
          </div>
          ...
          <div className="flex gap-2">
            <select
              ...
              className="rounded-lg bg-bg-primary border border-border px-3 py-1.5 text-xs text-text-secondary focus:outline-none"
            >
        ...
        <section className="space-y-6">
          {filteredQuestions.length > 0 ? (
            ...
            <div className="text-center py-12 rounded-2xl border border-border bg-bg-secondary">
              <HelpCircle className="h-8 w-8 text-slate-600 mx-auto mb-2" />
              <p className="text-sm text-text-muted">No matching correction questions found.</p>
            </div>
          )}
        </section>
      </main>
